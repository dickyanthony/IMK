...
const Inputs = forwardRef((props, ref) => {
  const { labelText = "input", className, error, ...restProps } = props;
      ...
      <div className={`${className} ${error && "animate-shake"} `}>
        <label className="relative cursor-text">
          <input
            ...
            type="text"
            placeholder="Input"
            className="h-12 w-80 px-4 text-base text-black bg-white border-black border-2 rounded-lg border-opacity-50 outline-none focus:border-blue-500 placeholder-gray-300 transition-all duration-100 placeholder-opacity-0 "
          />
